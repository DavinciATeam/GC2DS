...
import gc2.nlp.ligand.WALigand;
...
public class WABCell extends BCell {

	private static final long serialVersionUID = 1L;

	public WABCell(Anima anima) {
		super(anima);
	}

	@Override
	public boolean isTrigger() throws Exception {
		ILigand ligand = (ILigand) this.anima.getBDI().getBelief(NLPTerm.NLPLigand);
		if(ligand!=null){
			return true;
		}
		return false;
	...
	public IPlan selectPlan() {
		ILigand ligand = (ILigand) this.anima.getBDI().getBelief(NLPTerm.NLPLigand);
		return FeedbackPlanFactory.getInstance().generateFeedbackPlan(anima, new Feedback(ligand, new WALigand()));
	}
